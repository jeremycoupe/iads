...
for date in range(len(dateVecSummary)):
	if True:
		dfIads = pd.read_csv('~/Documents/iads/data/' + dateVecIADS[date] + '_iads_summary_0.5.csv' , sep=',' , index_col=False)
		...
		if str(dfIads['Runway Utilization At Start'][bankIndex]) == 'N_BE/A/T=36C':
			...
			totalFlights = dfIads['Total Flights In Broader Bank'][bankIndex]
			totalDepartures = dfIads['Flights In Departure Bank'][bankIndex]
			totalArrivals = dfIads['Flights In Arrival Bank'][bankIndex]
			numberHeld = dfSummary['Number Aircraft Held For Surface Metering'][0]
			overlap0 =  pd.Timestamp(dfIads['Start Arrival Bank'][bankIndex]) - pd.Timestamp(dfIads['Start Departure Bank Out'][bankIndex])
...
for i in range(1,16):
	plt.figure(i)
	plt.savefig('figs/utilization' + saveUtil + '_fig' + str(i) + 'FilterDays.png')
